...
git commit -m "feat: 新增 lib/episode_splitter 文字工具函式"
```
...
```bash
git add lib/episode_splitter.py tests/test_episode_splitter.py
git commit -m "feat: lib/episode_splitter 新增 peek_split / split_episode_text"
```
...
git add agent_runtime_profile/.claude/skills/manage-project/scripts/_text_utils.py agent_runtime_profile/.claude/skills/manage-project/scripts/split_episode.py
git commit -m "refactor: _text_utils 改 re-export 自 lib.episode_splitter（CLI 腳本輸出不變）"
```
...
```bash
uv run ruff check lib/ tests/ && uv run ruff format lib/ tests/
git add lib/project_manager.py tests/test_project_manager_more.py
git commit -m "feat: ProjectManager.commit_episode_split"
```
...
```bash
git add lib/episode_preprocess.py server/routers/projects.py tests/test_episode_split_skill.py
git commit -m "refactor: 抽出 lib/episode_preprocess.run_preprocess，preprocess 路由改呼叫它"
```
...
git add server/agent_runtime/skill_function_declarations.py tests/test_episode_split_skill.py
git commit -m "feat: gemini/openai 新增 peek_split_point / split_episode / preprocess_episode function"
```
...
```bash
git add agent_runtime_profile/.claude/skills/manage-project/SKILL.md agent_runtime_profile/CLAUDE.md
git commit -m "docs: SKILL.md/CLAUDE.md 同步分集切分 function 與角色提取引導"
```
...
```bash
uv run ruff check . && uv run ruff format --check .
git add server/routers/projects.py tests/test_episode_split_routes.py
git commit -m "feat: 新增 POST .../episodes/peek 與 .../episodes/split 路由"
```
...
```bash
cd /home/human/arcreel360
git add frontend/src/api.ts frontend/src/components/canvas/timeline/EpisodeSplitPanel.tsx frontend/src/components/canvas/timeline/EpisodeSplitPanel.test.tsx frontend/src/components/canvas/timeline/TimelineCanvas.tsx
git commit -m "feat: 前端分集切分面板 EpisodeSplitPanel + api.ts peek/split method"
```
