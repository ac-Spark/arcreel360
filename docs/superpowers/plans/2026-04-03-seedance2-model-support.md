...
**Goal:** 注册 Seedance 2.0 和 2.0 Fast 两個视频模型，添加定价规则和 per-model 能力映射，使用户可在配置中選用这两個模型进行 t2v/i2v 生成。
