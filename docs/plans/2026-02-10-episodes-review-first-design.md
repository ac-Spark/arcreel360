# 剧集/場景页「审片优先」改版设计（确认稿）

## 目标
- 将当前「剧集/場景」从“编辑驱动”调整为“审片驱动”。
- 在不改后端接口的前提下，把视频播放与分镜查看提升为首屏主任务。
- 保留現有保存、分镜生成、视频生成、上传、展开编辑能力，作为次级操作。

## 已确认决策
- 审片模式：单条播放（不做连播）。
- 播放器位置：页内主播放器（固定在列表上方）。
- 触发方式：点击場景卡中的视频缩略图开始播放。
- 审片布局：视频主画面 + 分镜副画面同时展示。
- 默认行为：进入页面后不自动播放，等待手动点击。
- 列表密度：5 列紧凑卡片。

## 现状问题（重心偏移）
- 当前每条場景卡中，视频与分镜仅为小尺寸角落预览，难以承担审片主任务。
- 页面视觉焦点集中在状态、文本与编辑按钮，而不是“看生成结果”。
- 小视频内嵌在列表中，播放时干扰浏览与切换，审片路径不连续。

## 设计方案

### 1) 页面结构
- 在 `ProjectEpisodes` 视图内，新增顶部主审片区，位于剧集列表之前。
- 主审片区默认空态文案：点击任意場景的视频缩略图开始审片。
- 主审片区激活后：
  - 左侧：16:9 大视频播放器（浏览器原生 controls）。
  - 右侧：对应場景分镜大图 + 关键信息（場景 ID、时长、状态）。
- 下方保留剧集卡和場景网格，承担“選片器”角色。

### 2) 場景卡策略
- 保留現有卡片信息与动作按钮（保存/分镜/视频/上传/展开编辑）。
- 卡片改为 5 列紧凑网格，维持高密度浏览效率。
- 点击卡片中的视频缩略图时，仅更新顶部主播放器，不在卡片内做主播放。
- 当前审片中的卡片高亮（边框/发光/标签），提升定位效率。

### 3) 参考 UI 取舍
- 采用参考图中的“深色主舞台 + 紧凑卡片矩阵”作为视觉基线。
- 差异点：新增固定主审片区，避免仅靠卡片浏览；保持“审片优先”。

## 组件与状态设计

### 组件层级（前端）
- 页面容器：`ProjectEpisodes`
- 新增组件建议：
  - `EpisodeReviewPanel`：顶部主审片区（空态/视频态）
  - `SceneGrid`：5 列場景卡网格（可复用現有渲染逻辑）
- 現有接口复用：
  - `resolveFileUrl(currentProjectName, path)` 解析视频与分镜地址
  - `onGenerateStoryboard` / `onGenerateVideo` / `onSaveItem` / `onUploadStoryboard`

### 新增状态
- `selectedReview: { scriptFile: string, itemId: string } | null`
  - 初始值为 `null`
  - 点击有视频的場景缩略图后写入
  - 主播放器根据该状态反查当前 item 和资产地址

## 数据流
1. 用户点击場景卡视频缩略图。
2. 前端检查该場景是否存在 `generated_assets.video_clip`。
3. 若存在：设置 `selectedReview`，主审片区加载并播放。
4. 若不存在：toast 提示“该場景暂无可播放视频”，不切换選中。
5. 主审片区同步展示同条分镜图（`generated_assets.storyboard_image`）与場景元信息。

## 错误处理与容错
- 選中失效：切换项目或数据刷新后，若選中项不存在则自动清空 `selectedReview`，回到空态。
- 视频不可用：主播放器 `onError` 时显示失敗提示，并给当前条“重新生成视频”入口。
- 重生成功自动刷新：若当前選中项重新生成成功，播放器地址自动使用最新 `video_clip`。
- 空数据兜底：无剧集、无場景、有場景但无视频，分别给出清晰空态文案。

## 测试策略
- 文件：`frontend/tests/*`（新增或扩展 `workspace-page` 相关测试）
- 覆盖最小回归集：
  - 默认进入页面显示主审片区空态，不自动播放。
  - 点击有视频缩略图后，主播放器渲染并绑定正确资源。
  - 点击无视频缩略图时仅提示错误，不切换選中态。
  - 当前選中卡片高亮可见。
  - 切换项目后選中态清空。
  - 主播放器加载失敗时显示错误态。

## 实施范围
- 主要修改：
  - `frontend/src/react/pages/workspace-page.js`
- 样式修改（若需要）：
  - `frontend/src/css/styles.css` 或 `frontend/src/css/app.css`
- 测试修改：
  - `frontend/tests/*workspace*`（按現有测试结构增补）

## 验收标准
- 进入「剧集/場景」后，顶部有主审片区，默认空态不自动播放。
- 点击任一有视频場景后，顶部出现大播放器并可正常播放。
- 主审片区同时显示同条分镜大图和場景关键信息。
- 下方保持 5 列紧凑場景网格，当前選中项有明确高亮。
- 原有生成/保存/上传/展开编辑操作仍可用且无回归。
