...
async def init_db() -> None:
    ...
    使用 Config() 空構造 + set_main_option 程式設計式呼叫 alembic，
    而非 Config("alembic.ini")，避免 env.py 的 fileConfig() 覆蓋應用日誌配置。
    """
    import asyncio
    ...
    def _run_alembic():
        # 程式設計式構造 Config，不讀 alembic.ini，
        # 從而跳過 env.py 的 fileConfig()，保護應用日誌配置
