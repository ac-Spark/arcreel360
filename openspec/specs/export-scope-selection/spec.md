...
- **THEN** ZIP 包含项目目录下所有文件（含 `versions/` 完整内容），与現有行为一致
...
当 `scope=current` 时，ZIP 中 SHALL 包含 `versions/versions.json` 文件，但内容经过裁剪：每個资源的 `versions` 数组仅保留 `current_version` 对应的那一条记录。
...
- **WHEN** 项目 storyboard E1S01 有 3 個版本（current_version=3），以 `scope=current` 导出
