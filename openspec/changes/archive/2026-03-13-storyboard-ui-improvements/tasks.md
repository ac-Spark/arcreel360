...
- [x] 1.2 在 Popover 中渲染 4s / 6s / 8s 三個選項按钮，当前值高亮
- [x] 1.3 選中新值后调用 `onUpdatePrompt(segmentId, "duration_seconds", newValue)` 并关闭 Popover
...
- [x] 3.2 `type === "location"` 时显示"場景"标签（amber 色调）；`type === "prop"` 时显示"道具"标签（emerald 色调）
