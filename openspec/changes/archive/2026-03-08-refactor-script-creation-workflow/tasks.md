...
- [x] 1.3 在 `settings.json` 的 `permissions.allow` 中添加两個新脚本的 Bash 执行权限
...
- [x] 2.5 用户完成 4 個 agent 创建后，审核生成的 agent 文件确保 frontmatter 和 system prompt 符合要求
...
- [x] 3.3 在 manga-workflow 中定义阶段间确认协议：每個 subagent 返回后展示摘要、使用 AskUserQuestion 获取用户确认、支持重做/跳过/继续
- [x] 3.4 在 manga-workflow 中定义上下文传递规则：每個 subagent dispatch 时传递什么参数（项目名、集数、content_mode、文件路径）
...
- [x] 5.3 更新 `agent_runtime_profile/CLAUDE.md`：替换工作流说明（从两個大 agent 的描述改为新的编排 skill + 聚焦 subagent 架构描述）
