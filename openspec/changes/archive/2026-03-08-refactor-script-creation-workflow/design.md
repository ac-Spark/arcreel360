...
ArcReel 使用 Claude Agent SDK 运行一個主 agent 会话，通过 Agent 工具（原 Task 工具）dispatch subagent 处理具体任务。当前有两個大型 subagent（`novel-to-narration-script`、`novel-to-storyboard-script`）各自包含 3-4 個步骤和用户确认点，以及一個静态编排文档 `manga-workflow`。
...
1. 将两個多步 subagent 拆解为多個聚焦的单任务 subagent，每個 subagent 做一件事并返回
...
- C) 每個原有步骤对应一個 subagent（5-6 個）→ 过度拆分，部分步骤太轻量不值得 subagent 开销

**理由**：3 個 subagent 对应 3 個真正需要推理的阶段（全局角色分析、单集预处理、JSON 生成时的验证和修正）。生成类操作（generate-characters 等）已有独立 skill/脚本，直接通过 subagent 调用即可。
...
**创建方式**：实施时由 Claude 提供每個 agent 的 description 文本，用户通过 `/agents` 命令使用 Claude 交互式创建。
...
- B) 拆分为多個独立 skill，用户手动按顺序调用 → 缺乏自动化编排，用户体验差
...
3. 每個 dispatch 返回后：展示摘要 → 用户确认 → 进入下一阶段
```
...
- 支持增量模式：如果 project.json 已有角色，subagent 对比小说和現有列表，只追加新角色
...
- 但没有独立的 CLI 脚本——当前两個大 agent 通过内嵌 Python 代码块调用这些方法
- subagent 需要通过 Bash 工具调用脚本（而非内嵌代码），因此需要一個可执行脚本
...
- 输出格式：Markdown（与現有 `step1_normalized_script.md` 格式一致，确保 script_generator 可无缝消费）
...
→ **缓解**：使用清晰的决策树结构和 Markdown 格式化；如果 prompt 过长，可拆分为多個辅助 skill。

### [风险] 断点续传能力变化
原来的大 subagent 有连续的 context，可以自然恢复。拆分后，中断恢复依赖编排 skill 重新检测状态。

→ **缓解**：每個阶段完成后都会持久化到文件系统（project.json、drafts/、scripts/），编排 skill 通过文件系统状态恢复。这实际上比原来的 subagent 内部恢复更可靠——不依赖 subagent context 存活。

### [Trade-off] Subagent 数量增加 → API 调用增加
原来 1 個 subagent 完成全部工作，现在 3-4 個 subagent 依次执行，每次 dispatch 都有开销。
