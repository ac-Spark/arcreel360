...
**Goals:**
- 将 Timeline 并发媒体请求数从 N（分镜总数）降低到视口可见数量 + overscan（约 8-12 個）
- 单個实体变更时，其他实体的媒体资源不被重新加载（覆盖全部 7 個消费组件）
...
- 无 UI 侵入性，仅提供 virtualizer hook，与現有 Tailwind 样式体系兼容
...
- **[entityRevisions 内存增长]** → 大项目可能有较多 key；实际上 100 分镜 + 20 角色 + 10 线索 ≈ 130 個 key，可忽略
...
- **[聚合通知文案截断]** → 超过 5 個同类变更时截断为"AI 新增了 5 個角色：张三、李四…等"
