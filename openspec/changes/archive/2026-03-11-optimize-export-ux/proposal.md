...
1. **下载阻塞**：导出使用前端 fetch + Blob 方式下载 ZIP，整個文件必须在内存中完成接收后才触发保存。期间用户无法看到进度，也不能切换页面（切走后 fetch 被中断）。对于包含大量分镜图/视频的项目，ZIP 可达数百 MB，体验极差。
2. **全量导出冗余**：每次导出都打包项目的全部内容（含 `versions/` 目录下的所有历史版本），数据量远超用户实际需要。多数場景下用户只需要当前版本的资源，不需要历史版本文件。
...
- **导出范围選項**：在导出交互中新增選择——"导出全部" 与 "仅当前版本"：
  - **导出全部**：行为与現有逻辑一致，打包整個项目目录（含 `versions/`）。
...
- `export-scope-selection`: 导出范围選择（全部 / 仅当前版本），包括后端打包逻辑和前端選項 UI
...
- **前端**：
  ...
  - `frontend/src/components/layout/GlobalHeader.tsx` — 导出按钮增加范围選择交互
