...
**理由**：原生 function calling 由 Gemini 服务端 grammar-constrained decoding 保证 JSON 合法性，无解析失败回退，也支持 parallel calls 与 streaming。Prompt-based 协议遇到长 args 时容易截断、JSON 半截、引号转义错。Gemini 1.5+ / 2.x / 3.x 全家族都支持原生 function calling，已稳定 18+ 個月。
...
**选择**：仿照 `BaseTextBackendRuntimeProvider._run_generation` 的 task 模式，在 `GeminiFullRuntimeProvider` 中实现 `_run_workflow_loop`：每轮调一次 `generate_content_stream` 拿到 functionCall，本地执行工具，把 functionResponse 喂回，循环直至模型回纯文本或主动 stop。message_buffer 与 SSE 流推送沿用現有协议。
...
**选择**：在 `server/agent_runtime/skill_function_declarations.py` 里把 7 個 skill 的输入参数表手写为 `FunctionDeclaration` 常量。每個 skill 一個函数，封装 `FunctionDeclaration(name=..., description=..., parameters=...)` 与「执行该 skill 时如何调用現有 service / pm / project_manager」的胶水代码。
...
**选择**：当用户拒绝一次工具调用时，不抛异常中断对话；而是构造一個 `functionResponse(name=<tool>, response={"error": "permission denied", "reason": "..."})` 喂回模型，让模型自己判断是要换路、放弃还是询问用户。
...
**选择**：在 `/settings` 助手区与新会话创建处，把現有 provider 单选改为：第一列选 provider（Gemini / OpenAI / Claude），第二列选模式（对话 / 工作流）；不可用组合（OpenAI × 工作流，因为本变更不实现 OpenAI full）以禁用态显示并提示「未实现」。
...
- **Q1**：parallel tool call 是否首期支持？Gemini 同一轮可能返回多個 functionCall，是串行执行还是并发？协作者评估：**首期建议串行执行**，简化错误处理，性能差距在 ArcReel 场景里不显著。
- **Q2**：`run_subagent` 工具的契约——是同步阻塞返回完整结果，还是异步返回 task_id 让模型轮询？**首期建议同步阻塞**（与 `manga-workflow` skill 現有协议一致），避免引入 task 状态机。
