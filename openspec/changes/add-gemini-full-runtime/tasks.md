...
- [x] 2.10 新建 `tests/test_skill_function_declarations.py`，22 個测试覆盖注册表完整性、4 個真实 skill 各 happy path + 校验失败、3 個占位 skill not_implemented、unknown_skill 拒绝、handler 异常隔离
...
- [x] 3.5 streaming token：实现 `_stream_one_turn` + `_gemini_stream`（用 `generate_content_stream` async iterator），文本 chunk 逐個 emit `stream_event` 给前端订阅者；fake SDK 不支持时自动回退 `_gemini_generate`；2 個 streaming 单测覆盖纯文本流与 functionCall+text 混合场景
...
- [ ] 7.3 PR 描述与协作者交接（待协作者完成 generate_storyboard/video/compose 三個占位 + UI 二维选择器后撰写）
