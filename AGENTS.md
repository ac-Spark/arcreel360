...
frontend/ (React SPA)  →  server/ (FastAPI)  →  lib/ (核心庫)
  React 19 + Tailwind       路由分發 + SSE        Gemini API
  wouter 路由               agent_runtime/        GenerationQueue
  zustand 狀態管理          (Claude Agent SDK)     ProjectManager
```
...
- `assistant.py` — Claude Agent SDK 會話管理（SSE 流式）
...
### Agent Runtime（Claude Agent SDK 整合）

`server/agent_runtime/` 封裝 Claude Agent SDK：
- `AssistantService` (`service.py`) — 編排 Claude SDK 會話
- `SessionManager` — 會話生命週期 + SSE 訂閱者模式
- `StreamProjector` — 從流式事件構建實時助手回覆
