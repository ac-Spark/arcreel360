...
class AssistantService:
    def __init__(self, project_root: Path):
        ...
        self.runtime_provider_registry: dict[str, AssistantRuntimeProvider] = {
            ...
            OPENAI_LITE_PROVIDER_ID: OpenAILiteProvider(
                project_root=self.project_root,
                data_dir=self.data_dir,
                meta_store=self.meta_store,
            ),
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        if meta := await self.meta_store.get(session_id):
            provider_id = meta.provider
        else:
            provider_id = CLAUDE_PROVIDER_ID

        if provider_id == CLAUDE_PROVIDER_ID and sdk_delete_session is not None:
            try:
                await asyncio.to_thread(sdk_delete_session, runtime_session_id(session_id))
            except Exception:
    ...
    async def _build_projector(
        ...
        history_messages = await self.session_manager.read_history_messages(meta.id)
    ...
    async def _with_session_metadata(
        ...
        *,
        ...
        if provider is not None:
            normalized["capabilities"] = provider.capabilities.model_dump()
